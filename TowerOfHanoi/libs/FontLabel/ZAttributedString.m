...
- (instancetype)initWithAttributedString:(ZAttributedString *)attr {
...
- (instancetype)initWithString:(NSString *)str attributes:(NSDictionary *)attributes {
    if ((self = [super init])) {
    ...
    for (NSUInteger i = 0; i <= _attributes.count; i++) {
        ...
        if (run != nil) {
            ...
            for (id key in run.attributes) {
                [attrDesc addObject:[NSString stringWithFormat:@"%@: %@", key, (run.attributes)[key]]];
            }
...
- (id)attribute:(NSString *)attributeName atIndex:(NSUInteger)index effectiveRange:(NSRangePointer)aRange {
    ...
    return [self attributesAtIndex:index effectiveRange:aRange uniquingOnName:attributeName][attributeName];
}
...
- (id)attribute:(NSString *)attributeName atIndex:(NSUInteger)index longestEffectiveRange:(NSRangePointer)aRange inRange:(NSRange)rangeLimit {
    NSParameterAssert(attributeName != nil);
    return [self attributesAtIndex:index longestEffectiveRange:aRange inRange:rangeLimit uniquingOnName:attributeName][attributeName];
}
...
- (void)enumerateAttributesInRange:(NSRange)enumerationRange options:(ZAttributedStringEnumerationOptions)opts
    ...
    if (NSEqualRanges(enumerationRange, NSMakeRange(0, 0))) {
        ...
        while (last > first+1) {
            ...
            ZAttributeRun *run = _attributes[pivot];
...
- (NSDictionary *)attributesAtIndex:(NSUInteger)index effectiveRange:(NSRangePointer)aRange uniquingOnName:(NSString *)attributeName {
    ...
    ZAttributeRun *run = _attributes[runIndex];
    if (aRange != NULL) {
        aRange->location = run.index;
        runIndex++;
        if (runIndex < _attributes.count) {
            aRange->length = [_attributes[runIndex] index] - aRange->location;
        } else {
...
- (NSDictionary *)attributesAtIndex:(NSUInteger)index longestEffectiveRange:(NSRangePointer)aRange
    ...
    if (aRange != NULL) {
        if (attributeName != nil) {
            id value = (run.attributes)[attributeName];
            ...
            while (1) {
                ...
                ZAttributeRun *prevRun = _attributes[runIndex];
                id prevValue = (prevRun.attributes)[attributeName];
            ...
            while (endRunIndex < _attributes.count) {
                ZAttributeRun *nextRun = _attributes[endRunIndex];
                if (nextRun.index >= NSMaxRange(rangeLimit)) {
                    endRun = nextRun;
                    break;
                }
                id nextValue = (nextRun.attributes)[attributeName];
            ...
            aRange->location = MAX(run.index, rangeLimit.location);
            ZAttributeRun *endRun = (runIndex+1 < _attributes.count ? _attributes[runIndex+1] : nil);
            aRange->length = MIN((endRun ? endRun.index : [_buffer length]), NSMaxRange(rangeLimit)) - aRange->location;
        }
    }
    return run.attributes;
...
- (NSRange)rangeOfAttributeRunsForRange:(NSRange)range {
    ...
    for (;;first++) {
        ...
        ZAttributeRun *run = _attributes[first];
    ...
    if (((ZAttributeRun *)_attributes.lastObject).index < NSMaxRange(range)) {
        ...
        while (firstAfter > lastIn + 1) {
            ...
            ZAttributeRun *run = _attributes[idx];
            if (run.index < range.location) {
                lastIn = idx;
            } else if (run.index > range.location) {
                firstAfter = idx;
        ...
        if ([_attributes[firstAfter] index] > NSMaxRange(range)) {
            // the first after is too far after, insert another run!
            ZAttributeRun *newRun = [[ZAttributeRun alloc] initWithIndex:NSMaxRange(range)
                                                              attributes:((ZAttributeRun *)_attributes[firstAfter-1]).attributes];
...
- (void)cleanupAttributesInRange:(NSRange)range {
    ...
    ZAttributeRun *lastRun = _attributes[range.location];
...
- (instancetype)initWithIndex:(NSUInteger)idx attributes:(NSDictionary *)attrs {
...
- (NSString *)description {
    ...
    for (id key in _attributes) {
        [components addObject:[NSString stringWithFormat:@"%@=%@", key, _attributes[key]]];
    }
